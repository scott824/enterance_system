	...
	try
	{
		...
		while(rs.next()) {
			...
			if(SosokName.compareTo("") == 0)
				SosokName = "대외부서";
			out.println("<tr>");
			out.println("<td class='userid'>" + rs.getString(1));
			out.println("</td>");
			out.println("<td class='sosokcode' id='"+rs.getString(2)+"'>" + SosokName);
			
			out.println("</td>");
			out.println("<td class='rankcode'>");
			psSosok = conn.prepareStatement("select rankname from avs11c1t where rankcode = '"+rs.getString(3)+"' ");
			rsSosok = psSosok.executeQuery();
			if(rsSosok.next()) {
				out.println(rsSosok.getString(1));
			}
			if(rsSosok != null) rsSosok.close();
			if(psSosok != null) psSosok.close();
			out.println("</td>");
			out.println("<td class='name'>" + rs.getString(4));
			out.println("</td>");
			out.println("</tr>");
		}
		if(rs != null ) rs.close();
